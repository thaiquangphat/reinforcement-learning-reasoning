...
class QueryPathActorCritic(nn.Module):
    """
    Actor–Critic baseline for KG path reasoning.
    - Single policy
    ...
    def run_episode(
        ...
        logps, values, next_values, rewards, idx_list = [], [], [], [], []
        success = False
        ...
        for _ in range(self.num_hops):
            ...
            # V(s_t)
            v_t = self.value(torch.cat([q, h_t], dim=0)).squeeze(0)
            ...
            if success:
                ...
                h_tp1 = rgat_nodes[next_idx].to(self.device)
                v_tp1 = self.value(torch.cat([q, h_tp1], dim=0)).squeeze(0)

            logps.append(logp)
            values.append(v_t)
            next_values.append(v_tp1)
            rewards.append(r_t)
