...
Hierarchical Query Path RL (Manager + Worker) with Path Encoder (GRU) and prototype subgoals.
Designed for long-hop multi-step navigation on relational graphs. Expects precomputed RGAT node embeddings
(rgat_nodes) supplied by the external GAT encoder. Compatible with existing dataset signatures.

Main class: HierarchicalQueryPathRLV1
- Manager chooses a subgoal prototype every K steps.
- Worker navigates for up to K steps conditioned on the chosen subgoal.
- Path encoder (GRU) maintains memory of visited node embeddings.
- Intrinsic reward (worker) shaped by distance-to-subgoal (cosine).
- run_episode returns trajectory useful for PPO-style updates.
...
class HierarchicalQueryPathRLV1(nn.Module):
    """
    ...
    def __init__(
        ...
        super(HierarchicalQueryPathRLV1, self).__init__()
        self.device = device
