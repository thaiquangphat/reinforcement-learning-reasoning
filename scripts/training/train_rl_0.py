"""
train_baseline_rl.py

Training script for a simple RL baseline (single policy, sparse reward)
for multihop QA on knowledge graphs.

- Reward = +1 if target reached, else 0
- No hierarchy, no intrinsic reward, no path encoder
...
def train(
    ...
    run_name = f"baseline_rl_{datetime.datetime.now():%Y%m%d_%H%M%S}"
