...
def train(
    ...
    save_path: str = "./checkpoints_baseline_ac",
    tag: str = "hotpotqa",
    encoder: str = "bert",
    ...
    run_name = f"baseline_rl_ac_{datetime.datetime.now():%Y%m%d_%H%M%S}"
    ...
    for epoch in range(epochs):
        ...
        for batch in pbar:
            ...
            for qury in batch["query"]:
                ...
                next_values = torch.stack(ep["next_values"])
                rewards = torch.tensor(ep["rewards"], device=device)

                td_error = rewards + gamma * next_values.detach() - values

                policy_loss = -(logps * td_error.detach()).mean()
                value_loss = F.mse_loss(values, rewards + gamma * next_values.detach())
