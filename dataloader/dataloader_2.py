...
class RelGraphDataset2(Dataset):
    ...
    def process_sample(self, sample):
        ...
        # Ensure that retrieval_path exists and is a list
        raw_paths = sample.get('retrieval_path', [])
        # Filter out empty paths early
        filtered_paths = [p for p in raw_paths if p and isinstance(p, list)]

        # If there are no non-empty retrieval paths, mark invalid
        if len(filtered_paths) == 0:
            valid = False
        else:
            # Check that all triplets in filtered paths reference known nodes
            for path in filtered_paths:
                for triplet in path:
                    if triplet[0] not in nodes or triplet[2] not in nodes:
                        valid = False
                        break
                if not valid:
                    break

            # check for recursive nodes (no repeated nodes in a path)
            if valid:
                for path in filtered_paths:
        ...
        if "amr" not in sample.keys():
            print("No AMR in sample")
        amr_clean = re.sub(r'~\d+', '', sample.get('amr', ''))  # clean ~#

        dct = {
            "valid": valid,
            "nodes": nodes,
            "edges": edges,
            "query": filtered_paths,            # return filtered (non-empty) paths
    ...
    def __getitem__(self, idx):
        ...
        nodes, edges, query, question, answer, amr = sample["nodes"], sample["edges"], sample["query"], sample["question"], sample["answer"], sample["amr"]

        # Defensive fallback: if query is empty or has only empty paths, synthesize a minimal path
        if not query or not any(isinstance(p, list) and len(p) > 0 for p in query):
            # create a synthetic trivial triplet using the first node as head/tail (no relation)
            # so training code indexing query[0][0] (start node) will not crash.
            if len(nodes) == 0:
                raise IndexError(f"Dataset sample {idx} has no nodes to build a fallback query.")
            synthetic_triplet = (nodes[0], "", nodes[0])
            query = [[synthetic_triplet]]
